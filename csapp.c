...
/**
 * @brief   Wrapper for the sigaction interface. Exits on error.
 * @param signum    Signal to set handler for.
 * @param action    Pointer to a 'struct sigaction' specifying the
 * new signal disposition.
 * @param old_action Pointer to a 'struct sigaction' to record the
 * previous signal disposition, or NULL.
 * @return  void
 */
void Sigaction(int signum, const struct sigaction *action,
...
/**
 * @brief   Set a signal handler. Exits on error.
 * @param signum    Signal to set handler for.
 * @param handler   Handler function.
 * @return void
 *
 * This function is provided for the common case where one wishes to
 * install a handler with sigaction flags equal to SA_RESTART and
 * an empty mask (i.e. block only the signal being handled) and
 * does not care what the previous disposition was.
 */
